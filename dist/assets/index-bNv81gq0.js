  ...
  filterResults(query = "") {
    if (!query.trim()) {
      ...
      this.filteredVideos = this.videos.filter(
        (video) => video.title.toLowerCase().includes(searchTerm) || video.description && video.description.toLowerCase().includes(searchTerm) || video.fullDescription && video.fullDescription.toLowerCase().includes(searchTerm)
      );
    }
    this.updateResultsCount();
...
`;
    markdown += `- **Analysis Date**: ${(/* @__PURE__ */ new Date()).toLocaleDateString()}
...
`;
  ...
  exportTitles() {
    ...
    const titlesText = this.filteredVideos.map((video) => video.title || "Untitled").join("\n");
...
class YouTubeApiService {
  constructor(apiKey) {
    ...
    this.rateLimiter = new DemoRateLimiter(CONFIG$1);
  }
  /**
   * Set the API key
  ...
  async getChannelIdFromHandle(handle) {
    var _a, _b;
    try {
      const response = await fetch(`${CONFIG$1.API.BASE_URL}/channels?part=id&forHandle=${handle}&key=${this.apiKey}`);
      ...
      const searchResponse = await fetch(`${CONFIG$1.API.BASE_URL}/search?part=snippet&type=channel&q=${handle}&key=${this.apiKey}&maxResults=1`);
  ...
  async getChannelData(channelInput) {
    ...
    const apiCall = async () => {
      const response = await fetch(`${CONFIG$1.API.BASE_URL}/channels?part=contentDetails&id=${channelId}&key=${this.apiKey}`);
    ...
    const data = this.performanceMonitor ? await this.performanceMonitor.trackApiCall("channels", apiCall()) : await apiCall();
  ...
  async getAllChannelVideos(uploadsPlaylistId, progressCallback = null) {
    ...
    let pageCount = 0;
    const maxResults = CONFIG$1.API.BATCH_SIZE;
    const videoLimit = this.isDemoMode ? CONFIG$1.DEMO.MAX_VIDEOS_PER_ANALYSIS : Infinity;
    let videosCollected = 0;
    do {
      let playlistUrl = `${CONFIG$1.API.BASE_URL}/playlistItems?part=snippet&playlistId=${uploadsPlaylistId}&maxResults=${maxResults}&key=${this.apiKey}`;
      if (nextPageToken) {
      ...
      const data = this.performanceMonitor ? await this.performanceMonitor.trackApiCall("playlistItems", apiCall()) : await apiCall();
      if (data.items && data.items.length > 0) {
  ...
  async processVideoDataBatched(items, progressCallback = null) {
    ...
    let videosData = [];
    const batchSize = CONFIG$1.API.BATCH_SIZE;
    for (let i = 0; i < items.length; i += batchSize) {
      ...
      try {
        const detailsResponse = await fetch(`${CONFIG$1.API.BASE_URL}/videos?part=snippet,statistics,contentDetails&id=${videoIds}&key=${this.apiKey}`);
  ...
  async validateApiKey(apiKey) {
    ...
    try {
      const testUrl = `${CONFIG$1.API.BASE_URL}/search?part=snippet&type=channel&q=test&maxResults=1&key=${apiKey}`;
  ...
  async getChannelAnalytics(channelId) {
    ...
    try {
      const response = await fetch(`${CONFIG$1.API.BASE_URL}/channels?part=snippet,statistics,brandingSettings,contentDetails&id=${channelId}&key=${this.apiKey}`);
      const data = await response.json();
      if (data.error) {
        throw new Error(data.error.message);
      }
  ...
  async makeApiCall(endpoint, params = {}) {
    ...
    const url = `${CONFIG$1.API.BASE_URL}/${endpoint}?${new URLSearchParams(allParams)}`;
...
class StorageService {
  ...
  saveSearch(searchData) {
    ...
    try {
      ...
      const maxSearches = ((_a = CONFIG$1.STORAGE) == null ? void 0 : _a.MAX_SAVED_SEARCHES) || 50;
...
class AnalyticsService {
  ...
  generateContentAnalysis() {
    ...
    const topVideo = this.videosData.reduce(
      (max, video) => (video.viewCount || 0) > (max.viewCount || 0) ? video : max
    );
  ...
  identifyViralContent() {
    ...
    const viralVideos = this.videosData.filter((video) => (video.viewCount || 0) >= viralThreshold).sort((a, b) => (b.viewCount || 0) - (a.viewCount || 0)).slice(0, 5);
  ...
  prepareUploadTimelineData() {
    ...
    return {
      ...
      datasets: [{
        ...
        data,
        borderColor: CONFIG$1.UI.CHART_COLORS.primary,
        backgroundColor: CONFIG$1.UI.CHART_COLORS.background,
        borderWidth: 2,
        fill: true,
        tension: 0.1
      }]
...
function detectEnvironment() {
  ...
  if (CONFIG$1.ENVIRONMENT.LOCAL_DOMAINS.includes(hostname) || hostname.startsWith("localhost")) {
    return "demo";
  }
  return "live";
}
function initializeEnvironment() {
  ...
  CONFIG$1.ENVIRONMENT.DETECTED = currentEnvironment;
...
function initializeDemoEnvironment() {
  CONFIG$1.DEMO.ENABLED = true;
  CONFIG$1.AUTH.GITHUB_OAUTH_ENABLED = false;
  {
...
class App extends BaseComponent {
  ...
  setupEventListeners() {
    ...
    if (analyzeBtn) {
      this.addListener(analyzeBtn, "click", (e) => {
        debugLog("🖱️ Analyze button clicked");
        if (this.performanceMonitor) {
          this.performanceMonitor.trackUserInteraction("analyzeButtonClick", {
      ...
      debugLog("✅ Analyze button listener attached");
    } else {
      debugLog("❌ Analyze button not found - this is critical!");
    }
    ...
    if (keywordInput) {
      ...
      debugLog("✅ Keyword input listeners attached");
    } else {
      debugLog("⚠️ Keyword input not found");
    }
    if (applyFilterBtn) {
      this.addListener(applyFilterBtn, "click", (e) => {
        debugLog("🖱️ Apply filter button clicked");
        if (this.performanceMonitor) {
          this.performanceMonitor.trackUserInteraction("applyFilterClick", {
            disabled: applyFilterBtn.disabled
          });
        }
        this.handleApplyFilter();
      });
      debugLog("✅ Apply filter button listener attached");
    } else {
      debugLog("⚠️ Apply filter button not found");
    }
  ...
  handleApplyFilter() {
    ...
    const channelName = ((_a = this.appState.channelData) == null ? void 0 : _a.channelTitle) || ((_c = (_b = this.appState.channelData) == null ? void 0 : _b.snippet) == null ? void 0 : _c.title) || "Unknown Channel";
  ...
  applyKeywordFilter(query) {
    ...
    const searchScope = ((_a = this.findElement('input[name="searchScope"]:checked')) == null ? void 0 : _a.value) || "both";
    const searchLogic = ((_b = this.findElement('input[name="searchLogic"]:checked')) == null ? void 0 : _b.value) || "OR";
    let keywords = [];
    if (query.includes(",")) {
  ...
  async handleAnalyzeChannel() {
    ...
    debugLog("🚀 Starting channel analysis", { channelQuery, hasApiKey: !!this.appState.apiKey });
  ...
  async analyzeChannel(channelQuery) {
    ...
    try {
      ...
      const channelName = channelData.channelTitle || ((_a = channelData.snippet) == null ? void 0 : _a.title) || "Unknown Channel";
  ...
  async initializeDemoMode() {
    try {
      const demoApiKey = "AIzaSyDemoKey_ForTestingPurposes_NotReal";
      this.setApiKey(demoApiKey);
      this.showInfo("Demo mode initialized - 50 video limit active");
      debugLog("🎭 Demo mode initialized with test API key");
    } catch (error) {
...
async function runIntegrationTests() {
  try {
    const { default: IntegrationTest } = await __vitePreload(async () => {
      const { default: IntegrationTest2 } = await import("./integration-test-E26Jn74k.js");
